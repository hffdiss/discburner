...
- **别在轮询里持续读原始扇区**：实测这台 USB 光驱在「每 6 秒按扇区读一遍整盘目录树」这种节奏下，
  几分钟后会直接从 USB 总线上掉下来（`drutil list` 变空、`ioreg -p IOUSB` 里也没有设备，
  得重新插拔才回来）。所以整盘内容读取只在**换盘 / 刻完之后**做一次（按
  `deviceNode + mediaID + 段数 + 介质` 的指纹判断），`chainState` 的原始扇区读也按区段布局缓存。
...
- **「盘上有什么」≠「系统让你看到什么」**（2026-09-21 实测，同一张 4 段 DVD+R）：
  Windows 按光盘 TOC 读**最后一段**（全部可见）；macOS 的 cd9660 驱动根本没有多区段概念，
  永远挂**第一段**；Linux 的 isofs 只有在光驱回应的 `READ TOC` 里能拿到「最后一段起始」时才读最后一段
  （`fs/isofs/inode.c` ← `drivers/scsi/sr_vendor.c` 的 `sr_cd_check()`），DVD+R 上不少光驱只回一条轨道，
  于是内核退回第一段。**数据没坏，是读的一方挑了旧的那一段**——凡是「追加后看不到文件」的报障，
  先按扇区读最后一段的目录树确认盘上到底有没有，别急着改刻录逻辑。
  Linux 上临时读全部内容的开关是 `-o sbsector=<最后一段起始扇区>`（`session=` 是轨道号，回环设备上会退化）。
- **可重写介质上的追加走「整盘合并重刻」**：读盘上全部内容 → 和这次要刻的合并（同名保留这次新加的）→
...
- 截图用的启动参数有顺序要求：解析时先找 `--demo-compatibility`，所以两者同时用要写成
  ...
  `--demo-rewritable` 把「这张盘可重写」当成真的，用来截「整盘合并重刻」那一行
  （只改判断，不会擦任何盘）。
