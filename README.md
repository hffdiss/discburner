...
- **通用格式**：默认生成 `ISO 9660 + Joliet + Rock Ridge` 混合光盘（用 `xorriso`），
  Windows / macOS / Linux / 车机都能读，中文文件名不乱码；没装 xorriso 时退回系统自带的
  `hdiutil makehybrid`（`ISO 9660 + Joliet + UDF 1.02`）
...
- **可追加（真·多区段嫁接）**：默认刻完**不关闭光盘**，同一张 DVD±R / BD-R 可以分多次刻。
  新段的目录树包含**之前所有区段里的文件**，Windows / Linux 上照样看得到旧内容
  （需要 xorriso，见下面「追加刻录到底怎么写」）
- **自动清理痕迹**：刻录前剔除 `.DS_Store`、`._*` 等 macOS 垃圾文件，避免它们出现在光盘里
- **进度看得懂**：刻录时状态栏写着「已用 X，预计还要 Y」，估算跑完会自动改成「正在收尾…」，不会停在一个说不清的「—」上
- **刻完报耗时**：完成弹窗和状态栏都会写清这一张盘花了多久（例如 `1 分 26 秒`），而不是只有一句「刻录完成」
- **外观可选**：设置里能把界面固定成浅色或深色，也可以跟随系统；版本号也在同一个面板里
...
```bash
./.build-manual/universal/discburn-selftest   # 348 项单元/集成自检
```
...
构建时还会顺手建一个按版本归档的目录：`dist/v1.2.0/` 里放着这一版的
`DiscBurner.app`、`.dmg`、`.zip` 和一份 `SHA256.txt`，方便回查「某个版本当时到底发的什么」。

只想看效果不想真发：加 `--dry-run`，它会把构建、自检和发布说明都跑一遍就停手。

## 图形界面

窗口按 macOS 的习惯组织：标题栏下方就是工具栏，**添加文件 / 添加文件夹 / 光盘内容 / 刷新**
这四个动作都在工具栏里；左栏是「盘里有什么 + 这次要刻什么」，右栏是可滚动的设置，
主操作「开始刻录」固定在窗口右下角，永远点得到。
...
这一栏可以收起（这一栏右上角的 `⌃`），中间的分隔线可以上下拖动调整高度；
点这一栏右上角的 ↑ 按钮或菜单「文件 → 查看光盘内容…」（`⌘I`）会打开完整窗口。
...
3. 把要刻的文件/文件夹拖进下面「要刻录的内容」，或用窗口工具栏的「添加文件 / 添加文件夹」
4. 看右栏的兼容性预检有没有要处理的文件名
5. 改卷标（光盘在「访达」里显示的名字）和刻录选项（可选）
6. 点「开始刻录」（固定在窗口右下角，`⌘↩` 也行）→ 确认 → 等进度条走完，光盘会自动弹出

刻录过程中状态栏会写「正在刻录… 已用 2 分 31 秒，预计还要 1 分 05 秒」；
刻完会直接告诉你这张盘还能不能再刻，并附上这次花了多久：弹窗和底部状态栏都写
「刻录完成 · 光盘仍可继续追加，还剩 X · 用时 1 分 26 秒」。
...
底栏右下角那颗小齿轮（或菜单「光盘刻录 → 设置…」，`⌘,`）打开设置面板。
里面能选**系统 / 浅色 / 深色**，选完立刻作用到所有窗口，并记住下次启动照旧；
面板底部写着当前版本号，和仓库根的 `VERSION` 文件、GitHub Release 上的 tag 完全一致。
平时这颗齿轮不抢注意力，需要的时候就在手边。
...
「通用数据光盘」现在用 `xorriso -as mkisofs` 生成 **ISO 9660 + Joliet + Rock Ridge**，
同一个卷里放了多套目录表，谁认得哪套就用哪套，所以三个系统都能读到同样的文件和中文名
（没装 xorriso 时退回系统自带的 `hdiutil makehybrid`，生成 `ISO 9660 + Joliet + UDF 1.02`）：

| 系统 | 读的是哪套 | 实际表现 |
| --- | --- | --- |
| Windows 7 及以后 | Joliet | 资源管理器直接打开，中文名、长文件名正常（名字上限 103 个字符） |
| Linux | Rock Ridge（内核 `isofs` 自带），没有就退到 Joliet | `mount /dev/sr0 /mnt` 即可，不需要装驱动，权限与符号链接也保留 |
| macOS | Rock Ridge | 插入即挂载，`diskutil info` 显示 `ISO Rockridge` |
| 车机 / 老播放器 / 老刻录机 | 只认 ISO 9660 时看到 8.3 大写名 | 这就是「名字太长」被标成警告的原因 |

几个容易被忽略的点：

- **多区段盘可以直接给别人**：新段的目录树里包含盘上**所有**旧区段的文件（旧文件按光盘绝对地址引用），
  Windows / Linux 挂上最后一段就能看到全部内容——现在这一版修的正是这件事，详见下面「追加刻录到底怎么写」。
  只有 1.2.0 及更早版本刻的**老盘**是「每段各自独立寻址」，那种盘追加后旧内容会消失，
  所以程序会直接拒绝继续追加（`discburn status` 里写「旧区段不兼容（旧版本刻的盘）」）。
  想在 Linux 上核对一张盘的全部区段，可以 `dd` 把整盘抽成映像，再用 `isoinfo -i disc.iso -R` 逐段看。
...
多区段光盘有个硬性约定：**每一段的目录树都要用「光盘绝对地址」，并且包含之前所有段里的文件。**
ISO 9660 的 Linux 实现里写得很直白：
*Multisession uses absolute addressing (solely the first frame of the whole track is #0)*。

如果像 1.2.0 那样「每次只写这一段新增的文件、地址从本段开头算起」，Windows / Linux 按规范
去读最后一段时就会指到错误的扇区，结果是**新文件和老文件都看不到**——这正是
「追加刻录出来的盘在 Windows / Linux 上读不到内容」的根因。对照实验（同一张盘、同一批文件）：

| 写法 | 新段根目录地址 | Windows / Linux 看到的内容 |
| --- | --- | --- |
| 每段独立寻址（1.2.0 及更早） | 41（区段相对，落回第 1 段的数据区） | 什么都没有 |
| 多区段嫁接（现在） | 546906（= 本段起始 546880 + 26，绝对地址） | 旧文件 + 新文件，全部可见 |

现在用的是 K3b、Nero 等刻录软件同一套做法：

```bash
# -M 旧段所在的盘，-C 上段起始扇区,下段起始扇区
xorriso -as mkisofs -M stdio:/dev/disk2 -C 483968,546880 \
  -input-charset UTF-8 -J -joliet-long -R -l -iso-level 3 \
  -o 新段.iso 要刻的内容/
```

几个细节：

- **旧文件不占新空间**：新段只是**引用**旧扇区，追加一段只花这一段自己的空间。
- **工具依赖**：`-M` 是 mkisofs 系工具的能力，系统自带的 `hdiutil` 做不到，需要
  `brew install xorriso`；装了 cdrtools 的 `mkisofs` 也能嫁接，但它写 Joliet 中文长名
  **只保留前 8 个字符**（自检里有对照实验），所以只当兜底，用了会提示换 xorriso。
- **读旧段有两条路**：优先直接让 xorriso 读光驱（卷挂载着会挡住访问，程序会先 `diskutil unmount`
  独占读盘）；读不了就按扇区把旧段抄成一张**稀疏映像**再交给它，这条路不依赖挂载状态。
- **刻之前自校验**：新段生成后会核对「根目录落在本段范围内（= 绝对地址）」和
  「目录树里真的引用了旧段里的文件」，任何一条不过就中止，不会把一张会让旧内容消失的盘刻出去。
- **老盘会被拒绝**：1.2.0 及更早版本刻的盘属于「区段相对寻址」，追加只会让旧内容在新段里消失，
  程序会明确报错并建议换空盘；旧盘上的内容仍然可以在 Mac 上读出来备份。

```bash
discburn status    # 段起始扇区 / 下一段起始 / 旧段可合并性 / 追加工具是否就绪
```
...
- 命令行与图形界面均能构建、运行；348 项自检全部通过（含速度建议、常用档位与耗时换算三组）
...
- **多区段嫁接（本轮重点）**：自检里用合成盘做对照——新段根目录落在本段范围内（绝对地址）、
  旧文件的地址仍指回第 1 段、旧文件内容逐字节核对无误；再用「按扇区抄出来的稀疏映像」当旧段
  接着刻第 3 段，最后一段里能同时看到第 1、2、3 段的文件（各 3~4 条断言）
- **真盘只读核对**：这台 DVD+R 的 11 段起始是 0 / 2592 / 5184 … 483968，下一段可写地址 546880；
  `xorriso -as mkisofs -M stdio:/dev/disk2 -C 483968,546880` 生成的新段根目录落在 **546906**
  （绝对地址，正确），而 1.2.0 的写法根目录是 **41**（区段相对，Windows / Linux 会指到第 1 段的数据区，
  所以新老文件都读不到）；这台盘因为是用旧版本刻的，程序会按设计拒绝继续追加
- **mkisofs 的 Joliet 缺陷对照实验**：同一个中文长名，xorriso 写出的 Joliet 名字完整，
  cdrtools 的 `mkisofs` 只保留前 8 个字符、其余补成 NUL；所以引擎首选 xorriso，mkisofs 只当兜底并会提示
...
- **这一轮追加的盘在 Windows / Linux 上其实读不到内容**（当时没发现）：那几段是「每段各自独立寻址」，
  新段的根目录地址落在第 1 段里，只有 macOS 靠挂载某一段能看到东西。根因与修法见
  「追加刻录到底怎么写（多区段嫁接）」一节，现在改成标准嫁接，并且刻之前会自校验。
